...
function initInputListening() {
    $('input, textarea').on('change', (e) => {
        ...
        else if ($(e.currentTarget).hasClass('stipend_time')) {
            studyYears = value;
        }
...
function initSubmit() {
    $('input.final-submit').unbind().on('click', () => {
        ...
        else if (!nation || !studyYears || !grade || -1 == courseNum || -1 == passNum || !reason || !identityCardId) {
...
// 测试
// downloadscholarshiptable();

function downloadscholarshiptable(mydata) {
    let date = new Date();
    let data = {
    ...
    //
    // let prizes = [
    //     {date: "2015年1", priceName: 'asdad', awardCeremony: '12313'},
    ...
    let form = $('<form id="downloadwordword" style="display:none" target="" method="post" action="/scholarshiptablesave"></form>');
    $('body').append(form);
    ...
    // form.append(`<input type="hidden" name="studyYears" value=${data.studyYears}/>`);
